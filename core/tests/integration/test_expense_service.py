...
def test_remove_expense_reduces_total():
    ...
    service.create_expense("Libro", 20, "Librería", date.today())
    service.create_expense("Revista", 5, "Quiosco", date.today())

    # Eliminamos el primer gasto (asumiendo ID 1 tras vaciar el repo)
    service.remove_expense(1)
...
def test_total_amount_after_removal():
    ...
    # Eliminamos el gasto "Cursos" (ID 1)
    service.remove_expense(1)
